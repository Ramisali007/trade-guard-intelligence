...
export const config = {
  ...
  processing: {
    /** Units per AI request. Batching many short units into one call is what makes large docs affordable. */
    unitsPerBatch: int('AI_UNITS_PER_BATCH', 12, 1, 60),
    /** Soft token ceiling for the text of a single batch; a batch closes when either limit is hit. */
    batchTokenBudget: int('AI_BATCH_TOKEN_BUDGET', 2800, 300, 40_000),
    /** Parallel in-flight AI requests per document. */
    concurrency: int('AI_CONCURRENCY', 4, 1, 32),
    /** Documents analysed concurrently across the whole process. */
    jobConcurrency: int('JOB_CONCURRENCY', 2, 1, 16),
    maxRetries: int('AI_MAX_RETRIES', 3, 0, 8),
    requestTimeoutMs: int('AI_TIMEOUT_MS', 120_000, 5_000, 600_000),
  ...
  ai: {
    ...
    anthropic: {
      ...
      maxOutputTokens: int('AI_MAX_OUTPUT_TOKENS', 8000, 512, 64_000),
    },
    openAiCompatible: {
      apiKey: openAiKey,
